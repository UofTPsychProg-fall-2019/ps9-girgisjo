...
ipip.l <- ipip.l %>% 
  separate(item, into=c('trait','item_num'), sep="_")


# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% 
  group_by(RID, trait) %>%
  summarise(mtrait=mean(score))
...
ipip.comp <- ipip.l %>% 
  ...
  left_join(ipip.comp, ipip.l, by = 'RID')
...
ipip.comp[, sapply(ipip.comp, class) == 'character'] #get variables which are character
ipip.comp <- mutate_at(ipip.comp, 'exer', ~factor(.))
ipip.comp[, sapply(ipip.comp, class) == 'factor'] #check that your variables now come up as factors

#reorder levels accordignto increasing frequency
ipip.comp$exer <- factor(ipip.comp$exer, levels = c('veryRarelyNever', 'less1mo',
...
exer.avg <- ipip.comp %>% 
  group_by(exer, trait) %>%
  summarise(avg=mean(mtrait), sem= sd(mtrait))
...
# then turn BMI_cat into a factor and order it with levels
ipip.comp$BMI_cat <- factor(ipip.comp$BMI_cat, levels = c('underweight', 'healthy',
                                                       'overweight', 'obese', 'other'))
...
age.avg <- ipip.comp %>% 
  group_by(trait)%>%
  summarise(corrcoef = cor(age, mtrait))
